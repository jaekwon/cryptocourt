// chainArguments' second half: the re-filing edges the realm now holds.
//
...
// The two reads chainArguments makes, answered per test.
let ARGS = null, SUP = null, CALLS = [];
global.one = async expr => {
  CALLS.push(expr);
  if(/ClaimArguments/.test(expr)){ if(ARGS===null) throw new Error("no such read"); return ARGS; }
...
eval(slice('async function chainArguments(', '\nfunction curationFor('));
...
(async ()=>{
  ...
  let rows = await chainArguments("orem", 7);
  ...
  rows = await chainArguments("orem", 7);
  ...
  CALLS = [];
  rows = await chainArguments("orem", 7);
  ok("argument edges survive alongside re-filings",
  ...
  // argument edges because a newer entrypoint is missing would make deploying
  // this overlay against an older realm strictly worse than not deploying it.
  ARGS = "out:2:s;in:"; SUP = null;
  rows = await chainArguments("orem", 7);
  ok("no ClaimSupersedes: argument edges still render",
     rows && rows.length===1 && rows[0][1]==="supported by this");
  ARGS = null; SUP = "of:5;by:";
  rows = await chainArguments("orem", 7);
  ok("no ClaimArguments: re-filings still render",
  ...
  CFG.mode='demo';
  ok("demo mode reads nothing", (await chainArguments("orem", 7)) === null);
  CFG.mode='live';
