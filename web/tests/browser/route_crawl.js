...
(async () => {
  ...
  // THE FORK CHIP, measured rather than eyeballed. It used to be a diagonal
  // ribbon in the bottom-right corner whose rotated band clipped its own glyph;
  // it is now a chip in the fixed top-right cluster beside the wallet button,
  // where the failure mode is different and just as invisible from source — two
  // fixed things in one corner overlapping each other. This page is already
  // open, so the check costs no browser.
  const corner = await page.evaluate(() => {
    const f = document.querySelector('.forkbtn');
    ...
    return {
      onScreen: r.width > 0 && r.left >= 0 && r.top >= 0
                && r.right <= innerWidth && r.bottom <= innerHeight,
      topRight: r.top < 60 && innerWidth - r.right < 220,
      clickable: !!(mid && mid.closest('.forkbtn')),
      overlapsWallet: hits(r, br),
      href: f.getAttribute('href'), rel: f.getAttribute('rel'),
    };
  });
  ok(`the fork chip is drawn fully inside the viewport`, corner.onScreen === true,
     JSON.stringify(corner));
  ok(`it sits in the top-right cluster`, corner.topRight === true);
  ok(`it is clickable at its own centre`, corner.clickable === true);
  ok(`and does not overlap the wallet button beside it`, corner.overlapsWallet === false);
